...
class TreeUtils
{
    ...
    public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = '')
    {
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                ...
                $this->ret .= $nstr;
                $nbsp = $this->nbsp;
                $this->get_tree($value['menuid'], $str, $sid, $adds . $k . $nbsp, $str_group);
                $number++;
            }
        }
        return $this->ret;
     ...
     * @return string
     */
    public function get_role_tree($myid, $str, $sid = 0, $adds = '', $str_group = '')
    {
        $number = 1;
        //一级栏目
        $child = $this->get_child($myid);
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                ...
                $this->ret .= $nstr;
                $nbsp = $this->nbsp;
                $this->get_role_tree($value['role_alias'], $str, $sid, $adds . $k . $nbsp, $str_group);
                $number++;
            }
        }
        return $this->ret;
